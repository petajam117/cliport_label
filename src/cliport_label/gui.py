...
class ToolGUI:
    ...
    def handle_keypress(self, key_press) -> None:
        ...
        if key_press == ord(self.controls['clear_all']):
            ...
            self.place_data_list = []
            self.lang_goal = ""
            rospy.loginfo("All task data cleared")
        ...
        if key_press == ord(self.controls['recover_robot']):
            self.task.recover()
        # Ignore task executor related commands if it's set to None
        if self.task is not None:
            ...
            if key_press == ord(self.controls['stop_execution']):
                ...
                self.task.stop()
            if self.task.is_valid and key_press == ord(self.controls['add_step']):
                if self.task.data_exists():
                    self.task.save_task_to_chain()
                    self.pick_data_list.append(self.pick_data)
                    self.place_data_list.append(self.place_data)
                    rospy.loginfo("task saved to chain")
                else:
                    rospy.logwarn("invalid task")
    ...
    def save_demo(self) -> None:
        ...
        if self.lang_goal == "":
            ...
            return
        elif self.snapshot is None or len(self.snapshot) != 2:
            ...
            self.pick_data_list.append(self.pick_data)
            self.place_data_list.append(self.place_data)
        if None in self.task.pick_pose[:-1] or None in self.task.place_pose[:-1]:
            rospy.logwarn("Action pair (pick-place) pose not found or chain contains broken steps")
            return
        
        action = {}
        pick_list = self.task.pick_pose[:-1]
        place_list = self.task.place_pose[:-1]
        ind = 0
        for pos in range(len(pick_list)):
            pick_key = f"pose{ind}"
            ind += 1
            place_key = f"pose{ind}"
            ind += 1

            action[pick_key] = pick_list[pos]
            action[place_key] = place_list[pos]

        # action = {'pose0': self.task.pick_pose, 'pose1': self.task.place_pose}
        color, depth = self.snapshot[0], self.snapshot[1]

        # NOTE: currently supporting only a single lang goal per multitask
        info = {'lang_goal': self.lang_goal, 'pick_data': self.pick_data_list, 'place_data': self.place_data_list}
        ...
        # Reset pick & place data for next demonstration
        self.pick_data = {'rotation': 0, 'bbox': [], 'rotline': []}
        self.pick_data_list = []
        self.place_data = {'rotation': 0, 'bbox': [], 'rotline': []}
        self.place_data_list = []
